    ...
    <div{if $OPTIONS.cart_zero_quantities=='yes'} class="orderlist"{/if}>
        ...
        <div id="row_{$i.id}" class="row cart-items">
            <div class="item-wrap clearfix">
                <div class="cart-item col-sm-5 col-xs-11"><span class="cart-itemname">{$i.name}</span>{if $i.description}<span class="cart-itemdescription"><br />{$i.description}</span>{/if}</div>
                ...
                <div class="cart-remove col-sm-1"><a class="remove" title="remove this item" href="cart/remove/{$i.id|escape:'url'}/" rel="nofollow" data-id="{$i.id}">x</a></div>
            </div>
        </div>
    {/foreach}
    </div>
